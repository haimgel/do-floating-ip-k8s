...
pub struct NodeInfo {
    pub droplet_id: i64,
    pub hostname: String,
    pub anchor_ip: String,
}

pub async fn node_info() -> Result<NodeInfo> {
    ...
    let res: DigitalOceanMetadata = reqwest::get(url).await?.json().await?;
    ...
    Ok(NodeInfo { droplet_id: res.droplet_id, hostname: res.hostname, anchor_ip })
}
